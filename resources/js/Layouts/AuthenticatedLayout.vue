...
<template>
    <div class="min-h-screen bg-stone-50 font-sans selection:bg-orange-500 selection:text-white relative">
        ...
        <nav class="bg-white border-b-3 sm:border-b-4 border-gray-900 sticky top-0 z-40 px-3 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto flex justify-between items-center h-14 sm:h-16">
                
                <!-- Brand & Role Badge -->
                <div class="flex items-center gap-2 sm:gap-3">
                    <Link href="/dashboard" class="text-lg sm:text-2xl font-black uppercase tracking-tighter hover:text-orange-500 transition">
                ...
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <template v-if="user.role === 'admin'">
                        ...
                        <div class="relative">
                            ...
                            <!-- Dropdown List Notifikasi (RESPONSIF MOBILE & DESKTOP) -->
                            <div
                                v-if="showingNotifDropdown"
                                class="fixed sm:absolute top-16 sm:top-auto right-3 sm:right-0 left-3 sm:left-auto z-50 mt-2 sm:mt-3 w-auto sm:w-80 rounded-2xl border-3 sm:border-4 border-gray-900 bg-white p-3 shadow-[4px_4px_0px_0px_rgba(17,24,39,1)] sm:shadow-[6px_6px_0px_0px_rgba(17,24,39,1)] space-y-2"
                            >
                                ...
                                <div v-else class="max-h-60 sm:max-h-64 overflow-y-auto space-y-2 pr-1">
                                    ...
                                    >
                                        ...
                                        <div class="leading-snug">
                                            <span class="font-black uppercase text-gray-900 block text-[11px] sm:text-xs">Produk Baru: {{ n.product?.name }}</span>
                        ...
                        <div class="relative">
                            <button
                                ...
                                type="button"
                                class="px-2.5 sm:px-4 py-1.5 sm:py-2 bg-amber-400 hover:bg-amber-300 border-2 border-gray-900 text-gray-900 font-black text-[10px] sm:text-xs uppercase tracking-wider rounded-xl shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] active:scale-95 transition flex items-center gap-1.5 sm:gap-2"
                            >
                                <span class="truncate max-w-[80px] sm:max-w-none">{{ user.username || user.name }}</span>
                                <svg 
                                    class="w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-200"
                            ...
                            <div
                                ...
                                class="absolute right-0 mt-2 w-48 sm:w-52 bg-white border-3 sm:border-4 border-gray-900 rounded-2xl shadow-[4px_4px_0px_0px_rgba(17,24,39,1)] sm:shadow-[6px_6px_0px_0px_rgba(17,24,39,1)] py-2 z-50 space-y-1"
                            >
                                <Link
                    ...
                    <template v-else>
                        <button
                            ...
                            type="button"
                            class="px-3 sm:px-4 py-1.5 sm:py-2 bg-rose-400 hover:bg-rose-300 border-2 border-gray-900 text-gray-900 font-black text-[10px] sm:text-xs uppercase tracking-wider rounded-xl shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] active:scale-95 transition"
                        >
                            Keluar
                        </button>
                    </template>
